...
COMMISSION_BPS_ROUNDTRIP: float = float(
    os.getenv("COMMISSION_BPS_ROUNDTRIP", "8")
)  # bps
# Доп. “микроиздержки” на проскальзывание — по желанию
SLIPPAGE_BPS: float = float(os.getenv("SLIPPAGE_BPS", "5"))

# === Риск-менеджмент ===
DEFAULT_RISK: float = float(
    os.getenv("DEFAULT_RISK", "0.005")
)  # 0.5% на сделку по умолчанию
