...
def rewrite_pending(pending: dict, done_ids: set):
    ...
    with open(PENDING_PATH, "w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(
            f,
            fieldnames=["order_id", "ticker", "figi", "lots", "stop", "target", "tick"],
        )
        w.writeheader()
        w.writerows(left)
    ...
    while True:
        try:
            ...
            with Client(token) as cli:
                ...
                for o in res:
                    oid = o.order_id
                    if oid in pending and o.execution_report_status.name in (
                        ...
                        try:
                            ...
                            log(
                                f"[STOPS] {row['ticker']}: SL/TP placed for order {oid}"
                            )
                            write_done(row)
                            done_ids.add(oid)
