...
with Client(token) as c:
    ...
    futures = {
        f.ticker: f
        for f in c.instruments.futures(
            instrument_status=InstrumentStatus.INSTRUMENT_STATUS_BASE
        ).instruments
    }
    ...
    with open(live_csv, newline="", encoding="utf-8") as f:
        ...
        for row in r:
            ...
            direction = (
                "long"
                if "long" in scen.lower()
                else ("short" if "short" in scen.lower() else "long")
            )
            ...
            if not getattr(instr, "api_trade_available_flag", False):
                continue
            if clazz == "stock":
                if direction == "long" and not getattr(
                    instr, "buy_available_flag", False
                ):
                    continue
                if direction == "short" and not getattr(
                    instr, "sell_available_flag", False
                ):
                    continue

            if getattr(instr, "trading_status", 0) not in (
