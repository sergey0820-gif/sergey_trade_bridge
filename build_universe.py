...
for s in shares_ru:
    mpi = (
        (s.min_price_increment.units + s.min_price_increment.nano / 1e9)
        if s.min_price_increment
        else None
    )
    rows.append(
        dict(
            ...
            mpi=mpi,
            currency=s.currency,
            is_traded=s.api_trade_available_flag,
        )
    )
for f in futs:
    mpi = (
        (f.min_price_increment.units + f.min_price_increment.nano / 1e9)
        if f.min_price_increment
        else None
    )
    rows.append(
        dict(
            ...
            name=f.name,
            exch=f.exchange,
            lot=(f.basic_asset_size or f.lot),
            mpi=mpi,
            currency=f.currency,
            is_traded=True,
        )
