...
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID') or os.getenv('CHAT_ID') or os.getenv('TELEGRAM_CHAT')
...
def send_signal(bot, row):
    ...
    buttons = [
        [InlineKeyboardButton("✅ Войти", callback_data=f"entry:{_getv(row,'symbol','ticker')}")],
        [InlineKeyboardButton("ℹ️ Детали", callback_data=f"info:{_getv(row,'symbol','ticker')}")],
        [InlineKeyboardButton("⛔ Пропустить", callback_data=f"skip:{_getv(row,'symbol','ticker')}")],
    ]
