...
- Для каждого тикера берём D1 и H4 свечи через Tinkoff Invest;
...
def get_figi_by_ticker(client: Client, ticker: str, class_code: str) -> str | None:
    """
    Ищем инструмент по тикеру/классу и возвращаем FIGI.
    """
    ...
    if not instruments:
        logger.warning("Не найден инструмент по тикеру %s", ticker)
        return None

    # фильтруем по class_code, если совпадает
    filtered = [i for i in instruments if i.class_code == class_code]
    if not filtered:
        inst = instruments[0]
    else:
        inst = filtered[0]
...
def scan_market(mode: str, max_tickers: int | None, notify: bool = False) -> None:
    ...
    with Client(token) as client:
        for idx, row in universe.iterrows():
            ...
            try:
                figi = get_figi_by_ticker(client, ticker, class_code)
                if not figi:
                ...
                h4_candles = fetch_candles(
                    client, figi, CandleInterval.CANDLE_INTERVAL_HOUR_4, days_h4
                )
    ...
    # notify сейчас оставим как флаг только для логов
    if notify:
