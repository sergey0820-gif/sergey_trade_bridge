...
def place_stop_order(
    ...
    try:
        ...
        logging.info(
            f"✅ Заявка размещена: {stop_order_type.name} для {instrument_uid}, цена триггера: {stop_price}"
        )
        return response
    except Exception as e:
        logging.error(
            f"🚨 Ошибка при размещении заявки {stop_order_type.name} для {instrument_uid}: {e}"
        )
        return None
