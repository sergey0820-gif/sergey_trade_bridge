...
if not TOKEN:
    print("❌ Нет TINKOFF_TOKEN в окружении")
    sys.exit(1)
...
def main():
    ...
    with Client(TOKEN) as c:
        shares = c.instruments.shares(
            instrument_status=InstrumentStatus.INSTRUMENT_STATUS_BASE
        ).instruments
        futures = c.instruments.futures(
            instrument_status=InstrumentStatus.INSTRUMENT_STATUS_BASE
        ).instruments
        ...
        for s in shares:
            ...
            if api_ok:
                summary["shares"]["api_ok"] += 1
            if buy_ok:
                summary["shares"]["buy_ok"] += 1
            if sell_ok:
                summary["shares"]["sell_ok"] += 1
            if norm:
                summary["shares"]["normal"] += 1
        ...
        for f in futures:
            ...
            if api_ok:
                summary["futures"]["api_ok"] += 1
            if norm:
                summary["futures"]["normal"] += 1
