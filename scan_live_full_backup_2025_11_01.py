...
def last_price(cli, figi):
    ...
    if lp: return float(lp[0].price.units) + lp[0].price.nano * 1e-9
    return None

# === Примитивные заглушки ===
def rsi_from_candles(candles): return 42.0
def trend_zone_d1(candles): return ("up", "support")
def scenario_and_levels(trend, rsi, price):
    if trend == "up" and rsi and rsi < 45:
        return ("Pullback → long", f"entry {price:.2f} / stop {price*0.98:.2f} / target {price*1.05:.2f}")
    if trend == "down" and rsi and rsi > 55:
        return ("Pullback → short", f"entry {price:.2f} / stop {price*1.02:.2f} / target {price*0.95:.2f}")
    return ("Pin off support → long", f"entry {price:.2f} / stop {price*0.98:.2f} / target {price*1.05:.2f}")
...
def pick_universe(cli: Client) -> list[Inst]:
    ...
    for s in shares:
        if s.exchange != "MOEX":
            continue
        res.append(Inst(s.ticker, s.class_code or "TQBR", s.figi, int(s.lot or 1), bool(s.for_qual_investor_flag)))
    futs = safe_call(cli.instruments.futures).instruments
    for f in futs:
        if f.basic_asset_size and f.basic_asset:
            res.append(Inst(f.ticker, f.class_code or "SPBFUT", f.figi, int(f.lot or 1), bool(f.for_qual_investor_flag)))
    scored = []
    for i in res:
        try:
...
def main():
    ...
    try:
        with Client(TOKEN) as cli:
            ...
            for inst in universe:
                try:
                    ...
                    row = {
                        ...
                        "scenario": scenario,
                        "recommendation": "watch long" if "long" in scenario else "watch short",
                        "levels": levels,
            ...
            log(f"Sergey-Trade: public={len(pub_rows)} | ki={len(ki_rows)} → {PUBLIC_CSV.name}, {KI_CSV.name}")
    except Exception as e:
