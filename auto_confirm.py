...
TINKOFF_TOKEN: Optional[str] = os.getenv("TINKOFF_TOKEN") or os.getenv(
...
def _side_from_zone(zone: str) -> Optional[str]:
    z = (zone or "").lower()
    if any(k in z for k in ("buy", "long", "support", "поддерж")):
        return "long"
    if any(k in z for k in ("sell", "short", "resist", "сопротив")):
        return "short"
    return None
...
def _ok_rsi(side: str, rsi_h4: Optional[float]) -> bool:
    if rsi_h4 is None:
        return False
    return (side == "long" and rsi_h4 <= RSI_LONG_MAX) or (
        side == "short" and rsi_h4 >= RSI_SHORT_MIN
    )


def _ok_dev(entry: float, last_px: float) -> bool:
    if not entry or not last_px:
        return False
    ...
    async with AsyncClient(TINKOFF_TOKEN) as cli:
        for r in rows:
            ...
            try:
                rsi_h4 = (
                    float(r.get("rsi_h4"))
                    if r.get("rsi_h4") not in (None, "", "—")
                    else None
                )
            except Exception:
                rsi_h4 = None
            if not _ok_rsi(side, rsi_h4):
                continue
            try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python auto_confirm.py out/live_candidates_public.csv [another.csv ...]"
        )
        raise SystemExit(0)
