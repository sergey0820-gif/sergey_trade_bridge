    ...
    for r in rows:
        ...
        try:
            plan = {
                ...
                "direction": (r.get("direction") or "").strip().lower(),  # 'long'/'short'
...
def main():
    ...
    ws_morn = open_or_create_worksheet(gc, sh, WS_MORNING, [
        "ts","ticker","class","trend_d1","zone","rsi_h4","scenario","recommendation","levels","all_in_bps","cost_r","pass"
    ])
    ws_live = open_or_create_worksheet(gc, sh, WS_LIVE, [
        "ts","ticker","class","trend_d1","zone","rsi_h4","scenario","recommendation","levels","all_in_bps","cost_r","pass"
    ])
    push_table(ws_morn, ["ts","ticker","class","trend_d1","zone","rsi_h4","scenario","recommendation","levels","all_in_bps","cost_r","pass"], csv_rows(MORNING_CSV))
    push_table(ws_live, ["ts","ticker","class","trend_d1","zone","rsi_h4","scenario","recommendation","levels","all_in_bps","cost_r","pass"], csv_rows(LIVE_CSV))
    print(f"[GS] pushed MORNING/LIVE at {ts_utc()}")

    # 2) APPROVALS → исполнение
    ws_app = open_or_create_worksheet(gc, sh, WS_APPROVE, [
        "ts","ticker","class","direction","entry","stop","target","qty","risk_pct","status","note"
    ])
    ensure_headers(ws_app, ["ts","ticker","class","direction","entry","stop","target","qty","risk_pct","status","note"])
    app_rows = ws_app.get_all_records()
    to_run = approvals_to_plans(app_rows)

    ws_exec = open_or_create_worksheet(gc, sh, WS_EXEC, [
        "ts","ticker","class","direction","entry","stop","target","qty","result","details"
    ])
    ...
    for src_row, plan in to_run:
        ...
        if TRADING_ENABLED:
            ...
            result, details = "DRY_RUN", "Trading disabled (DRY_RUN or TRADING_ENABLED=0)"

        # лог в EXECUTIONS
        ws_exec.append_row([
            ts_utc(),
            plan["ticker"], plan["class"], plan["direction"],
            plan["entry"], plan["stop"], plan["target"], plan.get("qty") or "",
            result, details
